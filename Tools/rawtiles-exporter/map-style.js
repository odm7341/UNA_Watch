export const layerGroups = Object.freeze({
  ...
  roads: ['roads-casing', 'roads', 'local-roads'],
  trails: ['trails', 'trail-labels'],
  peaks: ['peaks'],
  labels: ['place-labels', 'road-labels', 'local-road-labels']
});
...
export const style = {
  ...
  layers: [
    ...
    { id: 'local-roads', type: 'line', source: 'openfreemap', 'source-layer': 'transportation', minzoom: 12, filter: ['match', ['get', 'class'], ['minor', 'service'], true, false], layout: { 'line-cap': 'round', 'line-join': 'round' }, paint: { 'line-color': watchPalette.road, 'line-width': ['interpolate', ['linear'], ['zoom'], 12, 0.45, 15, 1.2, 19, 2] } },
    ...
    { id: 'local-road-labels', type: 'symbol', source: 'openfreemap', 'source-layer': 'transportation_name', minzoom: 15, filter: ['match', ['get', 'class'], ['minor', 'service'], true, false], layout: { 'symbol-placement': 'line', 'text-field': ['coalesce', ['get', 'name:en'], ['get', 'name']], 'text-font': ['Noto Sans Bold'], 'text-size': ['interpolate', ['linear'], ['zoom'], 15, 9, 19, 11] }, paint: { 'text-color': watchPalette.text, 'text-halo-color': watchPalette.paper, 'text-halo-width': 0.9 } }
  ]
};
